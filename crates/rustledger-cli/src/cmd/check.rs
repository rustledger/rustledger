...
fn run(args: &Args) -> Result<ExitCode> {
    ...
    for source_file in load_result.source_map.files() {
        let content =
            std::fs::read_to_string(&source_file.path).unwrap_or_else(|_| String::new());
        ...
        match load_error {
            LoadError::ParseErrors { path, errors } => {
                if args.quiet {
                    ...
                    error_count +=
                        report::report_parse_errors(errors, path, &source, &mut stdout)?;
                }
            }
    ...
    if args.auto
        && !native_plugins_to_run.contains(&"auto_accounts".to_string()) {
