...
fn run(args: Args) -> Result<()> {
    ...
    match args.command {
        ...
        Command::Region { file, start_line, end_line, conversion } => cmd_region(&file, start_line, end_line, conversion, &mut stdout),
    }
}
    ...
    for directive in linked {
        if let Directive::Transaction(txn) = directive {
            ...
            if !txn.links.is_empty() {
                writeln!(writer, "  Links: {}", txn.links.iter().map(|l| format!("^{l}")).collect::<Vec<_>>().join(" "))?;
            }
            for posting in &txn.postings {
                if let Some(amount) = posting.amount() {
                    writeln!(writer, "  {} {} {}", posting.account, amount.number, amount.currency)?;
                } else {
...
fn cmd_missing_open<W: Write>(file: &PathBuf, writer: &mut W) -> Result<()> {
    ...
    if missing.is_empty() {
        ...
        writeln!(writer, "; Missing Open directives ({} accounts)", missing.len())?;
        writeln!(writer)?;
...
fn cmd_list_options<W: Write>(writer: &mut W) -> Result<()> {
    ...
    let options = [
        ...
        ("operating_currency", "string", "Operating currencies (can be specified multiple times)"),
        ("render_commas", "bool", "Render commas in numbers"),
        ("name_assets", "string", "Name for Assets accounts (default: Assets)"),
        ("name_liabilities", "string", "Name for Liabilities accounts (default: Liabilities)"),
        ("name_equity", "string", "Name for Equity accounts (default: Equity)"),
        ("name_income", "string", "Name for Income accounts (default: Income)"),
        ("name_expenses", "string", "Name for Expenses accounts (default: Expenses)"),
        ("account_previous_balances", "string", "Account for opening balances"),
        ("account_previous_earnings", "string", "Account for previous earnings"),
        ("account_previous_conversions", "string", "Account for previous conversions"),
        ("account_current_earnings", "string", "Account for current earnings"),
        ("account_current_conversions", "string", "Account for current conversions"),
        ("account_unrealized_gains", "string", "Account for unrealized gains"),
        ("account_rounding", "string", "Account for rounding errors"),
        ("conversion_currency", "string", "Currency for conversions"),
        ("inferred_tolerance_default", "string", "Default tolerance for balance checks"),
        ("inferred_tolerance_multiplier", "decimal", "Multiplier for inferred tolerances"),
        ("infer_tolerance_from_cost", "bool", "Infer tolerance from cost"),
        ("documents", "string", "Directories to search for documents"),
        ("booking_method", "string", "Default booking method (STRICT, FIFO, LIFO, etc.)"),
        ("plugin_processing_mode", "string", "Plugin processing mode"),
        ("long_string_maxlines", "int", "Maximum lines for long strings"),
...
fn cmd_print_options<W: Write>(file: &PathBuf, writer: &mut W) -> Result<()> {
    ...
    if !options.operating_currency.is_empty() {
        writeln!(writer, "operating_currency: {:?}", options.operating_currency)?;
    }
    ...
    writeln!(writer, "account_previous_balances: {:?}", options.account_previous_balances)?;
    writeln!(writer, "account_previous_earnings: {:?}", options.account_previous_earnings)?;
    writeln!(writer, "account_current_earnings: {:?}", options.account_current_earnings)?;
...
fn cmd_stats<W: Write>(file: &PathBuf, writer: &mut W) -> Result<()> {
    ...
    writeln!(writer, "Directives:       {:>8}", load_result.directives.len())?;
    ...
    for spanned in &load_result.directives {
        match &spanned.value {
            ...
            Directive::Balance(bal) => {
                ...
                let entry = currency_scales.entry(bal.amount.currency.clone()).or_insert(0);
                if scale > *entry {
                    *entry = scale;
                }
            }
            Directive::Price(price) => {
                ...
                let entry = currency_scales.entry(price.amount.currency.clone()).or_insert(0);
                if scale > *entry {
                    *entry = scale;
                }
            }
...
fn cmd_roundtrip<W: Write>(file: &PathBuf, writer: &mut W) -> Result<()> {
    ...
    if !load_result.errors.is_empty() {
        writeln!(writer, "  Found {} parse errors in original", load_result.errors.len())?;
    }
    ...
    if !result2.errors.is_empty() {
        writeln!(writer, "  Found {} parse errors in round-trip", result2.errors.len())?;
    ...
    if original_count == roundtrip_count && result2.errors.is_empty() {
        writeln!(writer, "  SUCCESS: Round-trip produced same number of directives")?;
    } else {
        writeln!(writer, "  MISMATCH: Original had {original_count} directives, round-trip has {roundtrip_count}")?;
    }
...
fn cmd_directories<W: Write>(file: &PathBuf, dirs: &[PathBuf], writer: &mut W) -> Result<()> {
    ...
    writeln!(writer, "Validating directories against {} accounts", accounts.len())?;
    writeln!(writer, "{}", "=".repeat(60))?;
    writeln!(writer)?;
        ...
        for entry in walkdir(dir)? {
            ...
            if entry.file_type().is_dir() {
                ...
                if name.chars().next().is_some_and(char::is_uppercase) {
                    ...
                    let has_match = accounts.iter().any(|a| a.starts_with(&account_path) || a == &account_path);
                    if !has_match && !account_path.is_empty() {
                        writeln!(writer, "  WARNING: No matching account for directory: {account_path}")?;
                    }
                }
            }
        }
...
fn walk_dir_recursive(dir: &PathBuf, entries: &mut Vec<Result<DirEntry, std::io::Error>>) -> Result<()> {
    if dir.is_dir() {
            ...
            match entry {
                Ok(e) => {
                    ...
                    if let Ok(ft) = e.file_type() {
                        entries.push(Ok(DirEntry { path: path.clone(), file_type: ft }));
                        if ft.is_dir() {
...
fn cmd_region<W: Write>(file: &PathBuf, start_line: usize, end_line: usize, conversion: Option<Conversion>, writer: &mut W) -> Result<()> {
    let mut loader = Loader::new();
    let load_result = loader
        .load(file)
        .with_context(|| format!("failed to load {}", file.display()))?;
    ...
    writeln!(writer, "Transactions in region {}:{}-{}{}", file.display(), start_line, end_line, conversion_str)?;
    writeln!(writer, "{}", "=".repeat(60))?;
    writeln!(writer)?;
    ...
    if region_transactions.is_empty() {
        writeln!(writer, "No transactions found in lines {start_line}-{end_line}")?;
        return Ok(());
    }

    writeln!(writer, "Found {} transaction(s):", region_transactions.len())?;
    writeln!(writer)?;
                ...
                let (display_number, display_currency) = match conversion {
                    Some(Conversion::Cost) => {
                        ...
                        if let Some(ref cost) = posting.cost {
                            ...
                            let currency = cost.currency.clone().unwrap_or_else(|| amount.currency.clone());
                            (total_cost, currency)
                        } else {
                    ...
                    Some(Conversion::Value) => {
                        ...
                        writeln!(writer, "  (Note: value conversion requires price database, showing units)")?;
                        (amount.number, amount.currency.clone())
                    }
                    None => (amount.number, amount.currency.clone()),
                };
                writeln!(writer, "  {} {} {}", posting.account, display_number, display_currency)?;
                *balances.entry(format!("{}:{}", posting.account, display_currency)).or_default() += display_number;
            } else {
                writeln!(writer, "  {}", posting.account)?;
            }
        }
