...
fn run(args: &Args) -> Result<ExitCode> {
    if args.output.is_some() && args.files.len() > 1 {
        anyhow::bail!("--output can only be used with a single input file. Use --in-place for multiple files.");
    }
...
fn format_file(file: &PathBuf, args: &Args) -> Result<ExitCode> {
    ...
    if args.check {
        ...
        stdout.write_all(formatted.as_bytes()).context("failed to write to stdout")?;
        Ok(ExitCode::SUCCESS)
    }
}
