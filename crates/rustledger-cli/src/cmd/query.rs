...
fn write_text<W: Write>(result: &rustledger_query::QueryResult, writer: &mut W, numberify: bool) -> Result<()> {
    if result.columns.is_empty() {
        return Ok(());
    }

    // Calculate column widths
    let mut widths: Vec<usize> = result.columns.iter().map(std::string::String::len).collect();
...
fn write_csv<W: Write>(result: &rustledger_query::QueryResult, writer: &mut W, numberify: bool) -> Result<()> {
    // Print header
    writeln!(writer, "{}", result.columns.join(","))?;

    // Print rows
    for row in &result.rows {
        let values: Vec<String> = row.iter().map(|v| escape_csv(&format_value(v, numberify))).collect();
    ...
    loop {
        ...
        match readline {
            Ok(line) => {
                ...
                if matches!(lower.as_str(), "exit" | "quit" | "help" | "set" | "format" | "reload" | "errors" | "tables") {
                    eprintln!("warning: commands without \".\" prefix are deprecated. use \".{lower}\" instead");
            ...
            if args.is_empty() {
                ...
                match fs::read_to_string(query_file) {
                    Ok(query) => {
                        ...
                        let result = if let Some(ref output_path) = settings.output_file {
                            match fs::File::create(output_path) {
                                Ok(mut file) => execute_query(query, directives, settings, &mut file),
                                Err(e) => {
                                    eprintln!("error: failed to open {}: {}", output_path.display(), e);
                                    return;
                                }
