...
fn format_transaction(txn: &Transaction, config: &FormatConfig) -> String {
    ...
    for (key, value) in &txn.meta {
        writeln!(out, "{}{}: {}", &config.indent, key, format_meta_value(value)).unwrap();
    }
...
mod tests {
    ...
    fn test_format_simple_transaction() {
        let txn = Transaction::new(date(2024, 1, 15), "Morning coffee")
            ...
            .with_posting(Posting::new("Expenses:Food:Coffee", Amount::new(dec!(5.00), "USD")))
    ...
    fn test_format_balance() {
        let bal = Balance::new(date(2024, 1, 1), "Assets:Bank", Amount::new(dec!(1000.00), "USD"));
    ...
    fn test_format_open() {
        ...
        assert_eq!(
            formatted,
            "2024-01-01 open Assets:Bank:Checking USD,EUR\n"
        );
    }
