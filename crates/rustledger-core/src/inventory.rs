...
impl fmt::Display for BookingError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NoMatchingLot { currency, cost_spec } => {
...
impl Inventory {
    ...
    pub fn is_empty(&self) -> bool {
        self.positions.is_empty() || self.positions.iter().all(super::position::Position::is_empty)
    }
    ...
    fn reduce_strict(&mut self, units: &Amount, spec: &CostSpec) -> Result<BookingResult, BookingError> {
    ...
    fn reduce_strict_with_size(&mut self, units: &Amount, spec: &CostSpec) -> Result<BookingResult, BookingError> {
        let matching_indices: Vec<usize> = self
            .positions
            .iter()
            .enumerate()
    ...
    fn reduce_lifo(&mut self, units: &Amount, spec: &CostSpec) -> Result<BookingResult, BookingError> {
        self.reduce_ordered(units, spec, true)
    }

    /// HIFO booking: reduce from highest-cost lots first.
    fn reduce_hifo(&mut self, units: &Amount, spec: &CostSpec) -> Result<BookingResult, BookingError> {
    ...
    fn reduce_average(&mut self, units: &Amount) -> Result<BookingResult, BookingError> {
        ...
        self.positions.retain(|p| p.units.currency != units.currency);
    ...
    fn reduce_from_lot(&mut self, idx: usize, units: &Amount) -> Result<BookingResult, BookingError> {
        let pos = &self.positions[idx];
        let available = pos.units.number.abs();
        let requested = units.number.abs();
...
mod tests {
    ...
    fn test_reduce_insufficient() {
        ...
        assert!(matches!(result, Err(BookingError::InsufficientUnits { .. })));
    }
