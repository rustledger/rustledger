...
use rustledger_core::{
    ...
    IncompleteAmount, MetaValue, Metadata, Note, Open, Pad, Posting, Price, PriceAnnotation,
    Query, Transaction,
};
...
fn newline<'a>() -> impl Parser<'a, ParserInput<'a>, (), ParserExtra<'a>> + Clone {
    just('\n').ignored().or(just('\r').ignore_then(just('\n')).ignored())
}
...
fn build_cost_spec(components: Vec<CostComponent>, is_total_brace: bool) -> CostSpec {
    ...
    let hash_pos = components.iter().position(|c| matches!(c, CostComponent::Hash));
...
fn price_annotation<'a>() -> impl Parser<'a, ParserInput<'a>, PriceAnnotation, ParserExtra<'a>> + Clone
{
...
) -> impl Parser<'a, ParserInput<'a>, Option<PostingOrMeta>, ParserExtra<'a>> {
...
fn open_body<'a>(
    ...
    just("open")
        ...
        .ignore_then(account())
        .then_ignore(ws())
        .then(currency().separated_by(just(',').then(ws())).collect::<Vec<_>>())
        .then_ignore(ws())
    ...
    just("document")
        ...
        .map(move |(((acct, filename), tags_links), meta_items)| {
            let (tags, links): (Vec<_>, Vec<_>) = tags_links
                .into_iter()
                .map(|(t, l)| (t.into_iter().collect::<Vec<_>>(), l.into_iter().collect::<Vec<_>>()))
...
mod tests {
  ...
  Assets:Cash"#;
        ...
        if let Directive::Transaction(txn) = &result.directives[0].value {
            ...
            if let Some(MetaValue::Number(n)) = txn.meta.get("number-value") {
                ...
                panic!("number-value should be a number, got {:?}", txn.meta.get("number-value"));
            }
  ...
  Assets:Cash"#;
        ...
        if let Directive::Transaction(txn) = &result.directives[0].value {
            ...
            assert!(posting.meta.contains_key("document"), "missing document meta");
...
2024-01-17 * "Coffee" "Morning coffee"
  ...
  Assets:Cash"#;
        ...
        if let Directive::Transaction(txn) = &result.directives[0].value {
            assert!(txn.tags.contains(&"trip".to_string()), "first txn should have #trip tag");
        } else {
            panic!("expected transaction");
        }

        if let Directive::Transaction(txn) = &result.directives[1].value {
            assert!(txn.tags.contains(&"trip".to_string()), "second txn should have #trip tag");
        } else {
            panic!("expected transaction");
        }
        ...
        if let Directive::Transaction(txn) = &result.directives[2].value {
            assert!(!txn.tags.contains(&"trip".to_string()), "third txn should NOT have #trip tag");
        } else {
            panic!("expected transaction");
        }
    }
