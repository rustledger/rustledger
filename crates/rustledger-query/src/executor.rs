...
use crate::ast::{Query, SelectQuery, JournalQuery, BalancesQuery, PrintQuery, FromClause, Expr, Literal, BinaryOperator, FunctionCall, BinaryOp, UnaryOp, UnaryOperator, Target, OrderSpec, SortDirection};
...
impl<'a> Executor<'a> {
                    ...
                    if matches {
                        ...
                        let position_value = if let Some(at_func) = &query.at_function {
                            match at_func.to_uppercase().as_str() {
                                ...
                                "UNITS" => {
                                    posting.amount().map_or(Value::Null, |u| Value::Amount(u.clone()))
                                }
                                _ => posting.amount().map_or(Value::Null, |u| Value::Amount(u.clone())),
                            }
                        } else {
                            posting.amount().map_or(Value::Null, |u| Value::Amount(u.clone()))
                        };
                ...
                match (&op.left, &op.right) {
                    (Expr::Column(col), Expr::Literal(lit)) if col.to_uppercase() == "YEAR" => {
                        if let Literal::Integer(n) = lit {
                            ...
                            Ok(if op.op == BinaryOperator::Eq { matches } else { !matches })
                        } else {
                            Ok(false)
                        }
                    }
                    ...
                    (Expr::Column(col), Expr::Literal(Literal::Date(d))) if col.to_uppercase() == "DATE" => {
        ...
        match func.name.to_uppercase().as_str() {
            ...
            "PARENT" => {
                ...
                match val {
                    ...
                    _ => Err(QueryError::Type("PARENT expects an account string".to_string())),
                }
            }
            "LEAF" => {
                if func.args.len() != 1 {
                ...
                match val {
                    ...
                    _ => Err(QueryError::Type("LEAF expects an account string".to_string())),
                }
            }
            "ROOT" => {
                ...
                let n = if func.args.len() == 2 {
                    match self.evaluate_expr(&func.args[1], ctx)? {
                        ...
                        _ => return Err(QueryError::Type("ROOT second arg must be integer".to_string())),
                    }
                } else {
                ...
                match val {
                    ...
                    _ => Err(QueryError::Type("ROOT expects an account string".to_string())),
                }
            }
            "ACCOUNT_SORTKEY" => {
                ...
                match val {
                    ...
                    _ => Err(QueryError::Type("ACCOUNT_SORTKEY expects an account string".to_string())),
                }
            }
            "ABS" => {
                if func.args.len() != 1 {
            ...
            "SUBSTR" | "SUBSTRING" => {
                ...
                match (val, start, len) {
                    ...
                    _ => Err(QueryError::Type("SUBSTR expects (string, int, [int])".to_string())),
                }
            }
            "TRIM" => {
                if func.args.len() != 1 {
            ...
            "STARTSWITH" => {
                ...
                match (val, prefix) {
                    ...
                    _ => Err(QueryError::Type("STARTSWITH expects two strings".to_string())),
                }
            }
            "ENDSWITH" => {
                if func.args.len() != 2 {
            ...
            "ACCOUNT_DEPTH" => {
                ...
                match val {
                    ...
                    _ => Err(QueryError::Type("ACCOUNT_DEPTH expects an account string".to_string())),
                }
            }
            ...
            "NUMBER" => {
                ...
                match val {
                    ...
                    _ => Err(QueryError::Type("NUMBER expects an amount or position".to_string())),
                }
            }
            "CURRENCY" => {
                if func.args.len() != 1 {
                ...
                match val {
                    ...
                    _ => Err(QueryError::Type("CURRENCY expects an amount or position".to_string())),
                }
            }
            "GETITEM" | "GET" => {
                ...
                match (val, key) {
                    ...
                    _ => Err(QueryError::Type("GETITEM expects (inventory, string)".to_string())),
                }
            }
            "ROUND" => {
                if func.args.is_empty() || func.args.len() > 2 {
                ...
                let decimals = if func.args.len() == 2 {
                    match self.evaluate_expr(&func.args[1], ctx)? {
                        ...
                        _ => return Err(QueryError::Type("ROUND second arg must be integer".to_string())),
                    }
                } else {
                    0
                };
            ...
            "UNITS" => {
                ...
                match val {
                    ...
                    _ => Err(QueryError::Type("UNITS expects a position or inventory".to_string())),
                }
            }
            "COST" => {
                if func.args.len() != 1 {
                ...
                match val {
                    ...
                    _ => Err(QueryError::Type("COST expects a position or inventory".to_string())),
                }
            }
            "WEIGHT" => {
                ...
                match val {
                    ...
                    Value::Amount(a) => Ok(Value::Amount(a)),
                    _ => Err(QueryError::Type("WEIGHT expects a position or amount".to_string())),
                }
            }
            "VALUE" => {
                ...
                match val {
                    Value::Position(p) => {
                        ...
                        if p.units.currency == target_currency {
                            Ok(Value::Amount(p.units))
                        } else if let Some(converted) = self.price_db.convert(&p.units, &target_currency, date) {
                    ...
                    Value::Inventory(inv) => {
                        ...
                        for pos in inv.positions() {
                            ...
                            } else if let Some(converted) = self.price_db.convert(&pos.units, &target_currency, date) {
                    ...
                    _ => Err(QueryError::Type("VALUE expects a position or inventory".to_string())),
                }
            }
    ...
    fn evaluate_binary_op(
        ...
        match op.op {
            ...
            BinaryOperator::Regex => {
                ...
                let s = match left {
                    Value::String(s) => s,
                    _ => return Err(QueryError::Type("regex requires string left operand".to_string())),
                };
                let pattern = match right {
                    Value::String(p) => p,
                    _ => return Err(QueryError::Type("regex requires string pattern".to_string())),
                };
                ...
                match right {
                    Value::StringSet(set) => {
                        let needle = match left {
                            Value::String(s) => s,
                            _ => return Err(QueryError::Type("IN requires string left operand".to_string())),
                        };
                        Ok(Value::Boolean(set.contains(&needle)))
                    }
                    _ => Err(QueryError::Type("IN requires set right operand".to_string())),
                }
            }
    ...
    fn evaluate_unary_op(&self, op: &UnaryOp, ctx: &PostingContext) -> Result<Value, QueryError> {
        ...
        match op.op {
            ...
            UnaryOperator::Neg => match val {
                Value::Number(n) => Ok(Value::Number(-n)),
                Value::Integer(i) => Ok(Value::Integer(-i)),
                _ => Err(QueryError::Type("negation requires numeric value".to_string())),
            },
        }
    }
        ...
        F: FnOnce(Decimal, Decimal) -> Decimal,
    {
        let (a, b) = match (left, right) {
            ...
            (Value::Number(a), Value::Integer(b)) => (*a, Decimal::from(*b)),
            (Value::Integer(a), Value::Number(b)) => (Decimal::from(*a), *b),
            _ => return Err(QueryError::Type("arithmetic requires numeric values".to_string())),
        };
            ...
            let idx = match &spec.expr {
                Expr::Column(name) => {
                    column_indices.get(name.as_str()).copied().ok_or_else(|| {
                        QueryError::UnknownColumn(name.clone())
                    })?
                }
                Expr::Function(func) => {
                    // Try to find a column with the function name
                    column_indices.get(func.name.as_str()).copied().ok_or_else(|| {
