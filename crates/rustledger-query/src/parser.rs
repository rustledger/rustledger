...
use crate::ast::{Query, SelectQuery, Target, FromClause, Expr, OrderSpec, SortDirection, JournalQuery, BalancesQuery, PrintQuery, UnaryOperator, BinaryOperator, FunctionCall, Literal};
...
fn select_query<'a>() -> impl Parser<'a, ParserInput<'a>, SelectQuery, ParserExtra<'a>> {
    kw("SELECT")
        .ignore_then(ws1())
        .ignore_then(kw("DISTINCT").then_ignore(ws1()).or_not().map(|d| d.is_some()))
        .then(targets())
        ...
        .map(|((((((distinct, targets), from), where_clause), group_by), order_by), limit)| {
            SelectQuery {
                distinct,
                targets,
                from,
