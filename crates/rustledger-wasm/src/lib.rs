...
fn json_to_directive(json: &DirectiveJson) -> Result<Directive, String> {
    use rustledger_core::{Balance, Close, Commodity, CostSpec, Custom, Decimal, Document, Event, IncompleteAmount, Note, Open, Pad, Posting, Price, Transaction};
    ...
    match json.directive_type.as_str() {
        "transaction" => {
            let flag = json.data.get("flag")
                .and_then(|v| v.as_str())
                .and_then(|s| s.chars().next())
                .unwrap_or('*');
            let payee = json.data.get("payee").and_then(|v| v.as_str()).map(String::from);
            let narration = json.data.get("narration")
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .to_string();
            let tags: Vec<String> = json.data.get("tags")
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
                .unwrap_or_default();
            let links: Vec<String> = json.data.get("links")
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
                .unwrap_or_default();

            let postings_json = json.data.get("postings")
                .and_then(|v| v.as_array())
                .cloned()
                .unwrap_or_default();
            ...
            for p in postings_json {
                ...
                let units = if let Some(units_json) = p.get("units") {
                    if units_json.is_null() {
                        ...
                        let number = units_json.get("number")
                            .and_then(|v| v.as_str())
                            .and_then(|s| s.parse::<Decimal>().ok());
                        let currency = units_json.get("currency")
                            .and_then(|v| v.as_str())
                            .map(String::from);
                ...
                let cost = if let Some(cost_json) = p.get("cost") {
                    if cost_json.is_null() {
                        ...
                        let number_per = cost_json.get("number_per")
                            .and_then(|v| v.as_str())
                            .and_then(|s| s.parse::<Decimal>().ok());
                        let currency = cost_json.get("currency")
                            .and_then(|v| v.as_str())
                            .map(String::from);
                        let cost_date = cost_json.get("date")
                            .and_then(|v| v.as_str())
                            .and_then(|s| NaiveDate::parse_from_str(s, "%Y-%m-%d").ok());
                        let label = cost_json.get("label")
                            .and_then(|v| v.as_str())
                            .map(String::from);
        ...
        "balance" => {
            let account = json.data.get("account")
                .and_then(|v| v.as_str())
            ...
            Ok(Directive::Balance(Balance { date, account, amount, tolerance: None, meta: std::collections::HashMap::new() }))
        }
        "open" => {
            let account = json.data.get("account")
                .and_then(|v| v.as_str())
                ...
                .to_string();
            let currencies: Vec<String> = json.data.get("currencies")
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
                .unwrap_or_default();
            let booking = json.data.get("booking")
                .and_then(|v| v.as_str())
                .map(String::from);
            Ok(Directive::Open(Open { date, account, currencies, booking, meta: std::collections::HashMap::new() }))
        }
        "close" => {
            let account = json.data.get("account")
                .and_then(|v| v.as_str())
                ...
                .to_string();
            Ok(Directive::Close(Close { date, account, meta: std::collections::HashMap::new() }))
        }
        "commodity" => {
            let currency = json.data.get("currency")
                .and_then(|v| v.as_str())
                .ok_or("commodity missing currency")?
                .to_string();
            Ok(Directive::Commodity(Commodity { date, currency, meta: std::collections::HashMap::new() }))
        }
        "pad" => {
            let account = json.data.get("account")
                .and_then(|v| v.as_str())
                ...
                .to_string();
            let source_account = json.data.get("source_account")
                .and_then(|v| v.as_str())
                .ok_or("pad missing source_account")?
                .to_string();
            Ok(Directive::Pad(Pad { date, account, source_account, meta: std::collections::HashMap::new() }))
        }
        "event" => {
            let event_type = json.data.get("type")
                .and_then(|v| v.as_str())
                .ok_or("event missing type")?
                .to_string();
            let value = json.data.get("value")
                .and_then(|v| v.as_str())
                .ok_or("event missing value")?
                .to_string();
            Ok(Directive::Event(Event { date, event_type, value, meta: std::collections::HashMap::new() }))
        }
        "note" => {
            let account = json.data.get("account")
                .and_then(|v| v.as_str())
                ...
                .to_string();
            let comment = json.data.get("comment")
                .and_then(|v| v.as_str())
                .ok_or("note missing comment")?
                .to_string();
            Ok(Directive::Note(Note { date, account, comment, meta: std::collections::HashMap::new() }))
        }
        "document" => {
            let account = json.data.get("account")
                .and_then(|v| v.as_str())
                ...
                .to_string();
            let path = json.data.get("path")
                .and_then(|v| v.as_str())
                .ok_or("document missing path")?
                .to_string();
            Ok(Directive::Document(Document { date, account, path, tags: Vec::new(), links: Vec::new(), meta: std::collections::HashMap::new() }))
        }
        "price" => {
            let currency = json.data.get("currency")
                .and_then(|v| v.as_str())
            ...
            let amount = json_to_amount(amount_json)?;
            Ok(Directive::Price(Price { date, currency, amount, meta: std::collections::HashMap::new() }))
        }
        "query" => {
            let name = json.data.get("name")
                .and_then(|v| v.as_str())
                .ok_or("query missing name")?
                .to_string();
            let query = json.data.get("query")
                .and_then(|v| v.as_str())
                .ok_or("query missing query")?
                .to_string();
            Ok(Directive::Query(rustledger_core::Query { date, name, query, meta: std::collections::HashMap::new() }))
        }
        "custom" => {
            let custom_type = json.data.get("type")
                .and_then(|v| v.as_str())
                .ok_or("custom missing type")?
                .to_string();
            Ok(Directive::Custom(Custom { date, custom_type, values: Vec::new(), meta: std::collections::HashMap::new() }))
        }
...
fn json_to_amount(json: &serde_json::Value) -> Result<Amount, String> {
    ...
    let decimal: Decimal = number.parse()
...
"#;
        ...
        for spanned in &result.directives {
            ...
            let reconstructed = json_to_directive(&json)
                .expect("should reconstruct directive");
    ...
    fn test_validate_ledger_reconstruction() {
        ...
        let mut directives = Vec::new();
        for dir_json in &ledger.directives {
            let directive = json_to_directive(dir_json)
                .expect("should reconstruct directive");
        ...
        let validation_errors = validate_ledger(&directives);
        assert!(!validation_errors.is_empty(), "should detect Equity:Opening not opened");
    }
    ...
    fn test_all_directive_types_reconstruction() {
        ...
        for json in test_cases {
            ...
            assert!(result.is_ok(), "failed to reconstruct {}: {:?}", json.directive_type, result.err());
        }
    }
}
