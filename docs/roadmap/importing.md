...
| **Automatic balance extraction — OFX first** | `--balance` exists but the amount is hand-typed, so the assertion only catches *your* typos, not import gaps. **Phase 1 shipped:** the OFX parser reads `<LEDGERBAL>` and emits a `balance` directive, dated `DTASOF + 1` because a beancount assertion holds at the *start* of its date while a bank's figure is the close of business. CSV (phase 2) is still open. | Phase 1: surface the OFX ledger balance during extraction and feed it to the existing `reconcile()` — nearly free, and every OFX import becomes verified-complete. Phase 2: CSV via an institution-profile field. This is what turns importing from "hope it's complete" into "proven complete". |
| **Stable source IDs for dedup** | Dedup keys on exact date, exact first-posting amount, and a *fuzzy* payee/narration match (`rustledger-ops::dedup`). That last term degrades as the user curates: renaming a bank's `COFFEE SHOP` to "Blue Bottle" / "Morning latte" drops below the similarity threshold and the transaction is re-imported as a duplicate. Editing your ledger should not break dedup against the same source file. **Shipped in part:** the OFX parser reads `<FITID>` and emits it as an `^ofx-<id>` link, sanitized to the link charset so the output re-parses. Consuming it *in dedup* — preferring the id over the fuzzy key — is the remaining half. | Capture the source ID during extraction and prefer it over the fuzzy key when present, falling back to fuzzy matching for sources that have none (most CSV). Emit it as a `^link` rather than a `#tag`: it is identity, not a category, and thousands of unique values would swamp the tag namespace. Same OFX parsing pass as the `LEDGERBAL` row above. Also the precondition for merge-on-re-extract ([#2251](https://github.com/rustledger/rustledger/issues/2251)), which needs a stable identity to preserve hand edits across re-imports. Note `import-architecture.md` already specifies `ImportedTransaction.source_id`, but no such struct exists. |
